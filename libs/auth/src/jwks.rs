...
use xor_utils::{config::Config, http::http_client};
...
pub enum JwksClientError {
    /// No key found with given `key_id`
    #[error("No key found with the give key_id")]
    MissingKeyId,
...
impl JwksClient {
    /// Create a new isntance of the `JwksClient` with the provided config Arc reference
...
/// A convenience function to get a particular key from a key set and convert it to a secret
